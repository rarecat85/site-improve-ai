...
- `main`/`body`의 **직계 자식** 중 `isWireframeBlockCandidate`를 통과한 것만 1차 후보로 본다.
- 1차 후보가 **정확히 1개**일 때만 `unwrapSingleChildWrapperChain`이 동작한다. `div` / `section` / `article` / `aside`처럼 **구조적 래퍼** 안으로 **직계 요소 자식이 1개뿐**이면, 그 자식으로 한 단계 내려가며 **같은 규칙을 반복**한다(최대 **`MAX_SINGLE_WRAPPER_UNWRAP_DEPTH` = 14** 단계). AEM·레이아웃 그리드 등으로 여러 겹 감싼 뒤에야 형제 섹션이 나오는 경우, 상위에서 한 칸만 보이던 문제를 줄이기 위함이다.
- 래퍼 안으로 내려갔을 때 **직계 요소 자식이 2개 이상**이면:
  - 그중 `isWireframeBlockCandidate`를 통과한 노드가 **2개 이상**이면 → 그 노드들이 상위 블록 후보로 확정된다.
  - 통과가 **1개**뿐이면 → 그 노드만 잡고, 다시 위 **단일 자식 래퍼 언랩**을 이어 간다.
  - 통과가 **0개**이면 → 언랩을 멈춘다.
...
| `MAX_SINGLE_WRAPPER_UNWRAP_DEPTH` | 14 | 1차 후보가 1개일 때 단일 자식 래퍼를 따라 내려가는 최대 단계 |
