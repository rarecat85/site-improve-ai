...
개선 항목 본문은 **Lighthouse / axe / aiseo-audit** 결과를 프롬프트에 넣고 **OpenAI·Anthropic·Gemini**가 카테고리별로 생성합니다.
...
| 항목 | 도구 | Overview와의 관계 |
|------|------|-------------------|
| **CrUX** (실사용자 LCP 등) | **Chrome UX Report API** (`GOOGLE_CRUX_API_KEY` 선택) | `generateReport` 프롬프트·컨텍스트에 포함될 수 있으나, Overview 탭에 별도 위젯은 없음 |
