...
리포트 화면에 표시되는 **항목별 등급·상태**는 AI 추론이 아니라, `lib/utils/grade-calculator.ts`의 `computeDashboardGrades`에서 **규칙 기반**으로 계산합니다. Lighthouse·axe·HTTP 메타·aiseo-audit 결과를 0~100점으로 만든 뒤, 아래 표에 따라 **문자 등급(A+ ~ F)** 과 **한 줄 상태**를 붙입니다. (점수 자체의 정의·가중치는 그대로 두고, **등급 구간만** 실제 분포에 맞게 완화한 버전입니다.)
