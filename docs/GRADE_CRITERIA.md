...
2. **axe-core** 위반 **규칙별**로 원시 감점을 쌓습니다(노드 개수가 아니라 `violations[]` 항목 수). 규칙의 `impact`에 따라 가중합니다: **critical** 7 · **serious** 5 · **moderate** 2.8 · **minor** 1.2 · 알 수 없음 **2.5**. 가중 합은 **최대 24점**까지(원시 상한).
3. Lighthouse 접근성 점수가 **이미 높으면** axe 감점을 비율로 줄여, 두 도구가 비슷한 이슈를 중복 반영할 때의 **과한 감점**을 완화합니다: 점수 **≥83** → 감점 ×0.68 · **≥77** → ×0.78 · **≥71** → ×0.88 · 그 미만은 비율 없음. 이후 **최종 감점은 최대 20점**으로 한 번 더 상한을 둡니다.
4. 최종 접근성 점수 = `max(0, Lighthouse 접근성 점수 − 위 최종 감점)`.
5. Lighthouse 접근성 점수와 axe 조정값이 모두 없으면 카드는 **데이터 없음** 처리(등급 `—`).  
