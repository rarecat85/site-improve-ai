...
1. 우선 Lighthouse **PWA 카테고리** 점수.
2. 카테고리가 없으면 감사 평균:  
   `installable-manifest`, `service-worker`, `splash-screen`, `themed-omnibox`
3. 모두 없으면 카드에 **PWA 감사 없음** 등으로 표시(등급 `—`).
...
- SEO, 성능, 접근성(axe 조정 후), **모범 사례(Best practices)** , 보안, PWA, 모바일, 이미지, 스크립트
