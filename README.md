...
- **로컬호스트(`localhost` / `127.0.0.1`)** 분석 시: 라이브 배포에서 처리될 **전역 메타·구조화 데이터 성격**의 개선안은 되도록 제외하고 본문 중심을 권장하는 정책을 프롬프트에 덧붙입니다. (배경: **§1** — 라이브 vs 로컬만 있을 때 **본문만 구축한 전제**와 비교 지표를 맞추기 위함.)
...
전체 필드·금지 사항·우선순위 루브릭은 코드의 `getCategoryJsonRules`·`getSharedReportQualityRules`에 있으며, 탭별 입력 데이터는 [`docs/REPORT_CATEGORY_TABS.md`](docs/REPORT_CATEGORY_TABS.md)를 참고하세요.
...
- 선택(재현성·비교 조건): `ANALYSIS_FORM_FACTOR`, `LLM_TEMPERATURE`, `OPENAI_MODEL`, `ANTHROPIC_MODEL`, `GEMINI_MODEL`, `OPENAI_SEED` — [`docs/REPRODUCIBILITY.md`](docs/REPRODUCIBILITY.md)
