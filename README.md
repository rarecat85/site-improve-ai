...
환경 변수: `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY` (`.env.local`). 선택적으로 `GOOGLE_CRUX_API_KEY`로 실사용자 지표(CrUX)를 프롬프트·리포트에 반영합니다. **측정 폼 팩터**는 `ANALYSIS_FORM_FACTOR=desktop|mobile`(기본 `desktop`) — Lighthouse·Puppeteer 뷰포트·UA·스로틀링과 동일 프리셋으로 맞춤 (`lib/services/analyzer.ts`). **Lighthouse 단일 실행 상한(밀리초)** 은 `LIGHTHOUSE_TIMEOUT_MS`(미설정 시 기본 90000, 범위 30000~300000, `getLighthouseTimeoutMs()` in `lib/constants/analysis-pipeline.ts`).
...
| 성능·SEO 등 | **Lighthouse** | 카테고리별 감사 (로그는 `silent`로 터미널 노이즈 완화; **프로세스당 동시 1건** 직렬 — 아래 **§9** 참고) |
...
브라우저 (Next.js App Router)
  → POST /api/analyze (NDJSON 스트림: progress + 최종 report)
      → analyzeWebsite: Puppeteer + Lighthouse(직렬 락) + axe + 메타/DOM
      → (선택) CrUX, aiseo-audit
...
- 선택(재현성·비교 조건): `ANALYSIS_FORM_FACTOR`, `LIGHTHOUSE_TIMEOUT_MS`, `LLM_TEMPERATURE`, `OPENAI_MODEL`, `ANTHROPIC_MODEL`, `GEMINI_MODEL`, `GEMINI_FALLBACK_MODELS`, `LLM_FALLBACK_TO_GEMINI`, `OPENAI_SEED` — [`docs/REPRODUCIBILITY.md`](docs/REPRODUCIBILITY.md)
...
lib/
  ...
  storage/                # IndexedDB
  utils/                  # 등급·요약·비교 집계·Lighthouse 직렬 락(`lighthouse-mutex.ts`) 등
...
- **서버 부하**: 비교 시 `POST /api/analyze`는 **두 번 동시에** 올라가지만, **Lighthouse는 프로세스당 한 건씩만** 실행됩니다. 그래도 **Chrome(Puppeteer)은 URL당 하나씩** 떠 있으므로 메모리·CPU 부하는 남습니다. 여전히 무겁다면 재사용 캐시를 쓰거나, `LIGHTHOUSE_TIMEOUT_MS`로 상한을 조정하세요.  
